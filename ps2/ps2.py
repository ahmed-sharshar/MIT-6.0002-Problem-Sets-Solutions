...
def load_map(map_filename):
    ...
    file = open('F:\\E-JUST\\year3\\first term\\advanced programming' + map_filename, mode='r')
    d= Digraph()
    for line in file:
        edges=line.split()
        src = Node(edges[0])
        dest = Node(edges[1])
        total_dist = int(edges[2])    
        outdoor_dist = int(edges[3])
        x=WeightedEdge(src, dest, total_dist, outdoor_dist)
        if src not in d.nodes:
            d.add_node(src)
        if dest not in d.nodes:
            d.add_node(dest)
        if src in d.nodes and dest in d.nodes:
            d.add_edge(x)
    return d
