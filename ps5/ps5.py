...
def generate_models(x, y, degs):
    ...
    coefficient = []
    for i in degs:
        coefficient.append(pylab.polyfit(x, y, i))
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        ...
        degree = len(model) - 1
        R = r_squared(y, model_y)
        if degree == 1:
            standart_error = se_over_slope(x, y, model_y, model)
            titlestr = 'Climate Regression Model, Degree {0}\nR-squared: {1:3f}, SE/slope: {2:.3f}'.format(degree, R, standart_error)
        else:
            titlestr = 'Climate Regression Model, Degree {0}\nR-squared: {1:.3f}'.format(degree, R)
...
def gen_cities_avg(climate, multi_cities, years):
    ...
    for year in years:
        ...
        for city in multi_cities:
            annual_temp.append(get_city_avg(climate, city, year))
        cities_avg.append(sum(annual_temp)/len(multi_cities))
...
def gen_std_devs(climate, multi_cities, years):
    ...
    for year in years:
        ...
        std_dev.append(pylab.std(daily_temp))
